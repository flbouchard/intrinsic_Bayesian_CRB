# Taken from: https://github.com/alexandrebarachant/pyRiemann/blob/master/pyriemann/utils/base.py
import numpy
import scipy
...
def _matrix_operator(Ci, operator):
    ...
    Out = numpy.dot(numpy.dot(eigvects, eigvals), eigvects.conj().T)
    return Out
...
def sqrtm(Ci):
    ...
    """
    xsqrt = lambda x: numpy.sqrt(x)
    return _matrix_operator(Ci, xsqrt)
